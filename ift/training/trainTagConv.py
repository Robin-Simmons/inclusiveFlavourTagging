"""
Train a simple CNN model for Inclusive Flavour Tagging using track information.
...
import time
import pickle
import ift_tf_2.conv.convDefinition as convDefinition
...
def train(args):
    ...
    model = network(TRACK_SHAPE, args)
    ...
    model.compile(optimizer = adam, loss = BinaryCrossentropy(from_logits = args.logits), metrics=['accuracy'])
    ...
    plt.savefig("output_hist_float.pdf")
    plt.clf()    
    if args.logits == True:
        y_out_train = tf.keras.activtaions.sigmoid(y_out_train)
        y_out_test = tf.keras.activtaions.sigmoid(y_out_test)
    ...
    export_train_data(history.history, args.modelName)

    return model

    
def export_train_data(history, file_name):
     parse = [history["loss"], history["val_loss"], history["accuracy"], history["val_accuracy"]]
     np.savetxt("{}_history.txt".format(file_name), parse, delimiter = ",")
...
if __name__ == '__main__':
    ...
    argParser.add_argument("--nHidden", type = int, dest = "nHidden", default = 16, help = "Number of hidden units")
    ...
    argParser.add_argument("--logits", default = False, dest = "logits", help = "Set to true for quantisable networks")
    argParser.add_argument("--quantBits", type = int, default = 8, dest = "quantBits")
