...
def train(args):
    ...
    network = getattr(attentionDefinition, "attentionNetworkDropoutLayer")

    model = network(TRACK_SHAPE, args.units, args.dropout)
...
def evalModel(args, model, forceCPUTrue):
    ...
    with tf.device("/cpu:0"):
        for i in range(args.evalRepeats):
            start = time.time()
            y_out = model.predict_generator(genEval, verbose = 2)
    ...
    print("Units: {}, dropout rate: {}".format(args.units, args.dropout))
...
if __name__ == '__main__':
    ...
    argParser.add_argument("--units", type = int, dest = "units", default = 16, help = "Parameter to vary")
    argParser.add_argument("--dropout", type = float, dest = "dropout", default = 0.5, help = "Parameter to vary")
